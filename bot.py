...
def handle_logic(message):
    ...
    if u_text == 'TP-Link 🌐':
        ...
        try:
            ...
            bot.send_message(cid, "⚠️ تعذر تحميل الفيديو.")
        return
...
def validate_and_process(message):
    ...
    # --- الفلتر المحلي الذكي جداً ---
    
    # 1. فحص طول النص (يجب أن يكون أكثر من 10 حروف)
    if len(u_text) < 10:
        bot.send_message(cid, "⚠️ الرسالة قصيرة جداً! يرجى كتابة التفاصيل كاملة (الاسم والموقع ورقم التواصل).")
        return

    # 2. فحص وجود أرقام (للتأكد من وجود رقم هاتف أو رقم هوية)
    if not any(char.isdigit() for char in u_text):
        bot.send_message(cid, "⚠️ يرجى إدراج رقم للتواصل في الرسالة (رقم الجوال).")
        return

    # 3. فحص الرموز المتكررة (مثل النقاط فقط)
    if re.match(r'^[\W_]+$', u_text):
        bot.send_message(cid, "⚠️ يرجى كتابة نص واضح، الرموز والنقاط غير مقبولة.")
        return

    # إذا اجتاز الفحوصات، يتم الإرسال
